...
router.get('/', async (req, res) => {
  try {
    const fornecedores = await query('SELECT *, cnpj AS cpf FROM fornecedores ORDER BY nome');
...
router.get('/:id', async (req, res) => {
  try {
    const fornecedor = await queryOne('SELECT *, cnpj AS cpf FROM fornecedores WHERE id = ?', [req.params.id]);
...
router.post('/', async (req, res) => {
  const { nome, cnpj, cpf, telefone, email, endereco } = req.body;
  const doc = cpf || cnpj || '';
  if (!nome) return res.status(400).json({ erro: 'Nome obrigatório' });
  try {
    const result = await execute(
      'INSERT INTO fornecedores (nome, cnpj, telefone, email, endereco) VALUES (?, ?, ?, ?, ?)',
      [nome, doc, telefone || '', email || '', endereco || '']
    );
    res.status(201).json({ id: result.insertId, nome, cpf: doc, cnpj: doc, telefone, email, endereco });
  } catch (error) {
...
router.put('/:id', async (req, res) => {
  const { nome, cnpj, cpf, telefone, email, endereco } = req.body;
  const doc = cpf !== undefined ? cpf : (cnpj !== undefined ? cnpj : null);
  try {
    ...
    await execute(
      ...
      [nome ?? fornecedor.nome, doc ?? fornecedor.cnpj, telefone ?? fornecedor.telefone,
